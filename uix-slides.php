...
class UixSlides {
	...
	public static function frontpage_scripts() {
		...
		if( !self::tempfile_exists() ) {
			wp_enqueue_style( self::PREFIX . '-slides-frontend-style', self::plug_directory() .'theme_templates/uix-slides-style.css', false, self::ver(), 'all');
		} else {
	...
	public static function templates( $nonceaction, $nonce ){
		  ...
		  if ( self::wpfilesystem_connect_fs( $url, '', $contentdir, '' ) ) {
				...
				if ( self::tempfile_exists() ) {
					...
					return __( '<div class="notice notice-error"><p><strong>There was a problem copying your template files:</strong> Please check your server settings. You can upload files to theme templates directory using FTP.</p></div>', 'uix-slides' );
				}
